...
export default function SelectField({
  ...
  const { wrapperClassName, labelClassName } = classNames || {};
  return (
    <div
      className={cn(
        "flex w-full max-w-lg flex-col items-start",
        wrapperClassName
      )}
    >
      {label && (
        <label
          htmlFor={name}
          className={cn(
            "ml-1 text-sm font-medium text-foreground/60 mb-1",
            labelClassName,
            {
      ...
      >
        ...
        >
          ...
          <SelectContent>
            ...
            {options &&
              options?.map((item: unknown | any, index: number) => {
                const ItemValue = item?.key || item?.id || item;
                const ItemLabel = item?.name || item?.label || item;
                return (
